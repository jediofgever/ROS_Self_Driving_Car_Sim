...
class PriusHybridPluginPrivate {
   ...
   public:
    ...
    /// \brief Linear velocity of chassis c.g. in world frame at last update
    /// (m/s)
   public:
...
/////////////////////////////////////////////////
PriusHybridPlugin::PriusHybridPlugin() : dataPtr(new PriusHybridPluginPrivate) {
    int argc = 0;
    char *argv = nullptr;
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    this->dataPtr->node.Subscribe("/prius/reset", &PriusHybridPlugin::OnReset,
                                  this);
    this->dataPtr->node.Subscribe("/prius/stop", &PriusHybridPlugin::OnStop,
                                  this);

    this->dataPtr->node.Subscribe("/cmd_vel", &PriusHybridPlugin::OnCmdVel,
                                  this);
    this->dataPtr->node.Subscribe("/cmd_gear", &PriusHybridPlugin::OnCmdGear,
                                  this);
    this->dataPtr->node.Subscribe("/cmd_mode", &PriusHybridPlugin::OnCmdMode,
                                  this);

    this->dataPtr->posePub =
        this->dataPtr->node.Advertise<ignition::msgs::Pose>("/prius/pose");
    this->dataPtr->consolePub =
        this->dataPtr->node.Advertise<ignition::msgs::Double_V>(
            "/prius/console");

    std::string chassisLinkName =
        dPtr->model->GetName() + "::" + _sdf->Get<std::string>("chassis");
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->flWheelSteeringPID.SetPGain(
            _sdf->Get<double>(paramName));
    else
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->frWheelSteeringPID.SetPGain(
            _sdf->Get<double>(paramName));
    else
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->flWheelSteeringPID.SetIGain(
            _sdf->Get<double>(paramName));
    else
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->frWheelSteeringPID.SetIGain(
            _sdf->Get<double>(paramName));
    else
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->flWheelSteeringPID.SetDGain(
            _sdf->Get<double>(paramName));
    else
    ...
    paramDefault = 0;
    if (_sdf->HasElement(paramName))
        this->dataPtr->frWheelSteeringPID.SetDGain(
            _sdf->Get<double>(paramName));
    else
    ...
    ignition::math::Vector3d frCenterPos =
        this->dataPtr->frWheelJoint->GetChild()
            ->GetCollision(id)
            ->WorldPose()
            .Pos();
    ignition::math::Vector3d blCenterPos =
        this->dataPtr->blWheelJoint->GetChild()
            ->GetCollision(id)
            ->WorldPose()
            .Pos();
    ignition::math::Vector3d brCenterPos =
        this->dataPtr->brWheelJoint->GetChild()
            ->GetCollision(id)
            ->WorldPose()
            .Pos();
    ...
    this->dataPtr->handWheelPID.Init(100, 0, 10, 0, 0, handWheelForce,
                                     -handWheelForce);
    ...
    this->dataPtr->node.Subscribe("/keypress",
                                  &PriusHybridPlugin::OnKeyPressIgn, this);
}
...
void PriusHybridPlugin::KeyControlTypeA(const int _key) {
    ...
    switch (_key) {
        ...
        case 97: {
            ...
            this->dataPtr->handWheelCmd = std::min(
                this->dataPtr->handWheelCmd, this->dataPtr->handWheelHigh);
            this->dataPtr->lastSteeringCmdTime =
                this->dataPtr->world->SimTime();
            break;
        }
        // d - steer right
        case 68:
...
void PriusHybridPlugin::KeyControlTypeB(const int _key) {
    ...
    switch (_key) {
        ...
        case 115: {
            this->dataPtr->brakePedalPercent = 0.0;
            if (this->dataPtr->directionState !=
                PriusHybridPluginPrivate::REVERSE)
                this->dataPtr->gasPedalPercent = 0.0;
            this->dataPtr->gasPedalPercent += 0.1;
            this->dataPtr->gasPedalPercent =
                std::min(this->dataPtr->gasPedalPercent, 1.0);
...
void PriusHybridPlugin::OnStop(const ignition::msgs::Any & /*_msg*/) {
    ...
    bool executed = this->dataPtr->node.Request("/priuscup/upload", req,
                                                timeout, rep, result);
    if (executed) {
...
void PriusHybridPlugin::Update() {
    ...
    this->dataPtr->handWheelCmd = ignition::math::clamp(
    ...
    // this->dataPtr->handWheelJoint->SetPosition(0,
    // this->dataPtr->handWheelCmd); this->dataPtr->handWheelJoint->SetLowStop(0,
    // this->dataPtr->handWheelCmd);
    // this->dataPtr->handWheelJoint->SetHighStop(0,
    // this->dataPtr->handWheelCmd);
    ...
    this->dataPtr->flWheelSteeringCmd =
        atan2(tanSteer, 1 - this->dataPtr->frontTrackWidth / 2 /
                                this->dataPtr->wheelbaseLength * tanSteer);
    ...
    double brakePercent =
        this->dataPtr->brakePedalPercent + this->dataPtr->handbrakePercent;
    ...
    // Speed below medium-high threshold, throttle below low-medium threshold
             ...
             this->dataPtr->gasPedalPercent <=
                 this->dataPtr->kGasPedalLowMedium) {
    ...
    // threshold
    else if (this->dataPtr->evMode &&
             linearVel < this->dataPtr->speedLowMedium &&
             this->dataPtr->gasPedalPercent <=
                 this->dataPtr->kGasPedalMediumHigh) {
        // Gas engine is off, running on battery
        engineOn = false;
        batteryDischargePower = throttlePower;
    } else {
    ...
    if (engineOn && throttlePower > 0) {
        dPtr->gasConsumption +=
            dt * (dPtr->minGasFlow + throttlePower / dPtr->gasEfficiency /
                                         dPtr->kGasEnergyDensity);
    }

    // Accumulated mpg since last reset
    // max value: 999.9
    double mpg =
        std::min(999.9, dPtr->odom / std::max(dPtr->gasConsumption, 1e-6));
    ...
    if ((curTime - this->dataPtr->lastMsgTime) > .5) {
        ...
        else if (this->dataPtr->directionState ==
                 PriusHybridPluginPrivate::REVERSE)
        ...
        // EV mode
        this->dataPtr->evMode ? consoleMsg.add_data(1.0)
                              : consoleMsg.add_data(0.0);
